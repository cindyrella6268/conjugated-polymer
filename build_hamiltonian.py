    ...
    for i in range(N_local):
        for j in range(i+1, N_local):
            ...
            if abs(w) > tol_w:
                ...
                w_list.append([i ,j ,w])
    return H, pairs_added, total_w_added, np.array(w_list)
...
if __name__ == "__main__":
    ...
    onsite_energies = load_onsite_energies("onsite_eV.txt")
    ...
    if coords is None or normals is None:
        ...
        # 4) compute, add, and plot through-space couplings
        H, pairs_added, total_w, w_list = add_through_space_to_H(H, coords, normals, box_lengths, cutoff=through_space_cutoff)
        print(f"Through-space couplings added for {pairs_added} pairs (symmetric additions), total amplitude sum ~ {total_w:.6f} eV")

        # save w txt
        np.savetxt("w_pairs.txt", w_list, fmt="%d %d %.12e", header="i   j   w_ij_eV")
        import matplotlib.pyplot as plt

        w_vals = w_list[:, 2]       # extract w
        abs_w = np.abs(w_vals)      # magnitude

        bins = np.logspace(np.log10(abs_w.min()), np.log10(abs_w.max()), 60)

        plt.figure(figsize=(6,4))
        plt.hist(abs_w, bins=bins)
        plt.xscale("log")
        plt.xlabel("w_nm (eV)")
        plt.ylabel("Count")
        plt.tight_layout()
        plt.savefig("w_hist_logx.png", dpi=300)
        plt.close()
    ...
    np.savetxt("H_12082025.txt", H, fmt="%.12f")
