...
def extract_backbone_atoms_all_frames(filepath, selected_types={5, 6, 8}):
    frames = []
    box_bounds = []
    headers = []
        ...
        if "ITEM: TIMESTEP" in lines[i]:
            ...
            while i < len(lines) and not lines[i].startswith("ITEM:"):
                ...
                if atom_type in selected_types:
                    frame_atoms.append([atom_id, mol_id, atom_type, x, y, z])
                i += 1
...
#def wrap_coordinates(atoms, box_bounds):
 #   wrapped_atoms = []
 ...
 #   for atom in atoms:
 #       atom_id, mol_id, atom_type, x, y, z = atom
 #       xwrapped = x - np.floor((x - xlo) / (xhi - xlo)) * (xhi - xlo)
 #       ywrapped = y - np.floor((y - ylo) / (yhi - ylo)) * (yhi - ylo)
 #       zwrapped = z - np.floor((z - zlo) / (zhi - zlo)) * (zhi - zlo)
 #       wrapped_atoms.append([atom_id, mol_id, atom_type, xwrapped, ywrapped, zwrapped])
    ...
    with open(output_path, 'w') as f:
        for timestep, atoms in frames:
           # atoms = wrap_coordinates(atoms, box_bounds)
            ...
            f.write(f"ITEM: ATOMS id mol type x y z\n")
            for atom in atoms:
                f.write(" ".join(map(str, atom)) + "\n")

def main():
    input_file = 'dump.300hold.lammpstrj'
    output_file = 'backbone_only.dump'

    frames, box_bounds, headers = extract_backbone_atoms_all_frames(input_file, selected_types={5, 6, 8})
